...
Kuma uses the following default `ProxyTemplate` resource for every data plane proxy (`kuma-dp`) that is added to a [`Mesh`](/docs/{{ page.version }}/production/deploy/cluster). This resource looks like:
...
* The `selectors` object specifies the [data plane proxies](/docs/{{ page.version }}/production/dp) that are targeted by the `ProxyTemplate` resource. Values are provided as Kuma tags.
...
To customize the configuration of [data plane proxies](/docs/{{ page.version }}/production/dp), 
...
Modifications that Kuma applies on all [HTTP Connection Managers](https://www.envoyproxy.io/docs/envoy/latest/api-v3/extensions/filters/network/http_connection_manager/v3/http_connection_manager.proto.html#http-connection-manager-proto) in the Listener.

HTTP Filter modifications can only be applied on services [configured as HTTP](/docs/{{ page.version }}/features/traffic/protocol-support).
...
VirtualHost modifications can only be applied on services [configured as HTTP](/docs/{{ page.version }}/features/traffic/protocol-support).
...
At runtime, whenever `kuma-cp` generates the configuration for a given [data plane proxy](/docs/{{ page.version }}/production/dp), it will proceed as follows:

1. Kuma searches for all the `ProxyTemplates` resources that have been defined in the specified [`Mesh`](/docs/{{ page.version }}/production/deploy/cluster).
2. It loads in memory the `ProxyTemplates` resources whose `selectors` [match](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies) either an `inbound` or a `gateway` definition of any [data plane proxy](/docs/{{ page.version }}/production/dp) accordingly to the Kuma Tags selected.
3. Every matching `ProxyTemplate` is [ranked](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies). The `ProxyTemplate` resource with the highest ranking is used to generate the configuration for the specified data plane proxy (or proxies).
...
`ProxyTemplate` is a [Dataplane policy](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies). You can use all the tags in the `selectors` section.
