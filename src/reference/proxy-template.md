...
* The `selectors` object specifies the [data plane proxies](/docs/{{ page.version }}/explore/dpp/#dataplane-entity) that are targeted by the `ProxyTemplate` resource. Values are provided as Kuma tags.
...
To customize the configuration of [data plane proxies](/docs/{{ page.version }}/explore/dpp/#dataplane-entity), 
...
At runtime, whenever `kuma-cp` generates the configuration for a given [data plane proxy](/docs/{{ page.version }}/explore/dpp/#dataplane-entity), it will proceed as follows:
...
2. It loads in memory the `ProxyTemplates` resources whose `selectors` [match](/docs/{{ page.version }}/policies/how-kuma-chooses-the-right-policy-to-apply/) either an `inbound` or a `gateway` definition of any [data plane proxy](/docs/{{ page.version }}/explore/dpp/#dataplane-entity) accordingly to the Kuma Tags selected.
