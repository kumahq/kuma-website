...
Circuit breakers - unlike active [Health Checks](/docs/{{ page.version }}/features/health-check/health-check-policy) - do not send additional traffic to our data plane proxies but they rather inspect the existing service traffic. They are also commonly used to prevent cascading failures in our services.
...
`CircuitBreaker` is an [Outbound Connection Policy](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies).
