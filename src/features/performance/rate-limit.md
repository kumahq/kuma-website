...
When rate limiting to an [ExternalService](/docs/{{ page.version }}/production/networking/external-services), the policy is applied per sending service instance.`
## Usage
...
`RateLimit`, when applied to a dataplane proxy bound Kuma service, is an [Inbound Connection Policy](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies).

When applied to an [ExternalService](/docs/{{ page.version }}/production/networking/external-services), `RateLimit` is an [Outbound Connection Policy](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies). In this case, the only supported value for `destinations.match` is `kuma.io/service`.
