...
This policy provides **active** checks. If you want to configure **passive** checks, please utilize the [Circuit Breaker](/docs/{{ page.version }}/features/performance/circuit-breaker) policy. Data plane proxies with **active** checks will explicitly send requests to other data plane proxies to determine if target proxies are healthy or not. This mode generates extra traffic to other proxies and services as described in the policy configuration.
...
`HealthCheck` is an [Outbound Connection Policy](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies).
