...
- Security between our services, via the [mTLS policy](/docs/{{ page.version }}/features/zero-trust/mutual-tls).
...
{% tip %}
This section is not to be confused with the [mTLS policy](/docs/{{ page.version }}/features/zero-trust/mutual-tls) that we can apply to a [Mesh](/docs/{{ page.version }}/production/deploy/cluster) to secure service-to-service traffic.
{% endtip %}
...
See [Data plane proxy authentication](/docs/{{ page.version }}/production/security/dp-auth) and [Zone Ingress authentication](/docs/{{ page.version }}/features/multi-zone/zone-ingress-auth).
...
See [API Server authentication](/docs/{{ page.version }}/production/security/api-server-auth).
...
{% tabs configure-zone-control-plane useUrlFragment=false %}
...
{% endtab %}
{% tab configure-zone-control-plane Universal %}
Point to the certificate and the key.
```sh
