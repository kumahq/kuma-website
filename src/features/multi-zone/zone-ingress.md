...
To implement cross-zone communication when Kuma is deployed in a [multi-zone](/docs/{{ page.version }}/production/deployment-topologies/multi-zone) mode, there is a new proxy type `ZoneIngress`.
...
The recommended way to deploy a `ZoneIngress` proxy in Kubernetes is to use `kumactl`, or the Helm charts as specified in [multi-zone](/docs/{{ page.version }}/production/deployment-topologies/multi-zone). It works as a separate deployment of a single-container pod.
