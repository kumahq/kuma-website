...
zones or [external services](/docs/{{ page.version }}/production/networking/external-services) in the local zone).
...
* All requests that are sent from local data plane proxies to [external services](/docs/{{ page.version }}/production/networking/external-services)
...
`kumactl`, or the Helm charts as specified in [multi-zone](/docs/{{ page.version }}/production/deployment-topologies/multi-zone).
...
Multi-zone deployment is similar and for deployment, you should follow [multi-zone deployment instruction](/docs/{{ page.version }}/production/deployment-topologies/multi-zone).
...
[mTLS](/docs/{{ page.version }}/features/zero-trust/mutual-tls) is required to enable `ZoneEgress`. In addition, there's a configuration in the `Mesh` policy to route traffic through the `ZoneEgress`
