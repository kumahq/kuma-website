...
If you haven't already read the [observability docs](/docs/{{ page.version }}/features/observability).
{% endtip %}
...
`TrafficLog` is an [Outbound Connection Policy](/docs/{{ page.version }}/features/policy-enforcement/choosing-policies).
...
Internally, Kuma [determines traffic protocol](/docs/{{ page.version }}/features/traffic/protocol-support) based on the value of `kuma.io/protocol` tag on the `inbound` interface of a `destination` `Dataplane`.
