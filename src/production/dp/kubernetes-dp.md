...
On Kubernetes the [`Dataplane`](/docs/{{ page.version }}/production/dp) entity is automatically created for you, and because transparent proxying is used to communicate between the service and the sidecar proxy, no code changes are required in your applications.
...
* `kuma.io/zone`: Identifies the zone name in a [multi-zone deployment](/docs/{{ page.version }}/production/deployment-topologies/multi-zone).
* `kuma.io/protocol`: Identifies [the protocol](/docs/{{ page.version }}/features/traffic/protocol-support) that was defined by the `appProtocol` field on the Service that selects the Pod.
