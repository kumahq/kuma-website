...
* `kuma.io/zone` identifies the zone name in a [multi-zone deployment](/docs/{{ page.version }}/production/deployment-topologies/multi-zone). This tag is automatically created and cannot be overwritten.
* `kuma.io/protocol` identifies [the protocol](/docs/{{ page.version }}/features/traffic/protocol-support) of the service exposed by this inbound. Accepted values are `tcp`, `http`, `http2`, `grpc` and `kafka`.
...
This is only useful when not using [transparent-proxy](/docs/{{ page.version }}/production/networking/transparent-proxying). 
...
- Kubernetes: The control plane [**automatically generates**](/docs/{{ page.version }}/production/dp/kubernetes-dp) the `Dataplane` entity. 
- Universal: The [**user defines**](/docs/{{ page.version}}/production/dp/universal-dp) the `Dataplane` entity. 
 
For more details check the [dataplane specification](/docs/{{ page.version }}/reference/resources/proxy_dataplane):
