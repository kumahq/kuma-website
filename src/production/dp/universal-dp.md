...
When transparent proxying is not enabled, the outbound service dependencies have to be manually specified in the [`Dataplane`](/docs/{{ page.version }}/production/dp) entity.
...
{% tip %}
Note that in Universal dataplanes need to start with a token for authentication. You can learn how to generate tokens in the [security section](/docs/{{ page.version }}/production/security/dp-auth).
{% endtip %}
...
By default, a proxy will be considered healthy regardless of its state. Consider using [service probes](/docs/{{ page.version }}/features/health-check/service-health-probes)
...
Consider using [service probes](/docs/{{ page.version }}/features/health-check/service-health-probes) to mark data plane proxy as unhealthy when it is in draining state.
...
`Dataplane` resource is created using [HTTP API](/docs/{{ page.version }}/reference/http-api#dataplanes) or [kumactl](/docs/{{ page.version }}/reference/cli).
...
If the `admin` section is empty or port is equal to zero then the default value for port will be taken from the [Kuma Control Plane configuration](/docs/{{ page.version }}/reference/cmd/kuma-cp/kuma-cp).
