...
The usage of Kuma DNS is only relevant when [transparent proxying](/docs/{{ page.version }}/production/networking/transparent-proxying) is used.
...
Follow the instruction in [transparent proxying](/docs/{{ page.version }}/production/networking/transparent-proxying).
 ...
 * The mode can safely be used with the [Kuma CNI plugin](/docs/{{ page.version }}/production/networking/cni).
 * In mixed IPv4 and IPv6 environments, it's recommended that you specify an [IPv6 virtual IP CIDR](/docs/{{ page.version }}/production/networking/ipv6).
...
The `serviceVipEnabled` field defines if there should be a vip generated for each `kuma.io/service`. This can be disabled for performance reason and [virtual-outbound](/docs/{{ page.version }}/production/networking/outbound-communication) provides a more flexible way to do this.
...
The preferred way to define hostnames is using [Virtual Outbounds](/docs/{{ page.version }}/production/networking/outbound-communication).
