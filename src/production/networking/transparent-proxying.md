...
Kuma integrates with a service naming provided by Kubernetes DNS as well as providing its own [Kuma DNS](/docs/{{ page.version }}/production/networking/dns) for multizone service naming.

## Universal

On **Universal** `kuma-dp` leverages the [data plane proxy specification](/docs/{{ page.version }}/reference/resources/proxy_dataplane) associated to it for receiving incoming requests on a pre-defined port.

There are several advantages for using transparent proxying in universal mode:

 * Simpler [Dataplane resource](/docs/{{ page.version }}/reference/resources/proxy_dataplane), as the `outbound` section becomes obsolete and can be skipped.
 * Universal service naming with `.mesh` [DNS domain](/docs/{{ page.version }}/production/networking/dns) instead of explicit outbound like `https://localhost:10001`.
 * Support for hostnames of your choice using [VirtualOutbounds](/docs/{{ page.version }}/production/networking/outbound-communication) that lets you preserve existing service naming.
...
Kuma comes with [`kumactl` executable](/docs/{{ page.version }}/reference/cli) which can help us to prepare the host. Due to the wide variety of Linux setup options, these steps may vary and may need to be adjusted for the specifics of the particular deployment.
...
At the same time, you can now connect to services using [Kuma DNS](/docs/{{ page.version }}/production/networking/dns).
...
You can also control this value on whole Kuma deployment with the following Kuma CP [configuration](/docs/{{ page.version }}/production/cp)
```
