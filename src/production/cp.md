...
All possible configuration and their default values are in the [`kuma-cp` reference doc](/docs/{{ page.version }}/reference/cmd/kuma-cp/kuma-cp).
...
When Kuma (`kuma-cp`) is up and running it needs to store its state somewhere.
