...
`MeshGateway` is a policy used to configure [Kuma's builtin gateway](/docs/{{ page.version }}/production/gateway/built-in).
It is used in combination with [`MeshGatewayRoute`](/docs/{{ page.version }}/production/gateway/builtin-routes).
...
The reference doc contains all options on [`MeshGateway`](/docs/{{ page.version }}/production/gateway/builtin-listeners).
