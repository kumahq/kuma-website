...
Kuma exposes API server on [ports](/docs/{{ page.version }}/production/networking/default-ports) `5681` and `5682` (protected by TLS).
...
This group is [authorized by default](/docs/{{ page.version }}/production/security/api-access-control) to execute all administrative operations.
...
3. Disable localhost is admin (optional)
   ...
   After you retrieve and store the admin token, [configure a control plane](/docs/{{ page.version }}/production/cp) with `KUMA_API_SERVER_AUTHN_LOCALHOST_IS_ADMIN` set to `false`.
   {% endtab %}
   {% endtabs %}

### Generate user tokens

You can generate user tokens only when you provide the credentials of a user [authorized to generate user tokens](/docs/{{ page.version }}/production/security/api-access-control#generate-user-token).
...
2. Disable bootstrap of the token
   [Configure a control plane](/docs/{{ page.version }}/production/cp) with `KUMA_API_SERVER_AUTHN_TOKENS_BOOTSTRAP_ADMIN_TOKEN` set to `false`.
   {% endtab %}
