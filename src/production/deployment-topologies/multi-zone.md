...
In the presence of a [zone egress](/docs/{{ page.version }}/features/multi-zone/zone-egress) the traffic is routed through the local zone egress before being sent to the remote zone ingress.

When using [transparent-proxy](/docs/{{ page.version }}/production/networking/transparent-proxying) (enabled by default in Kubernetes), Kuma generates a VIP, a DNS entry with the format
`<kuma.io/service>.mesh`, and will listen for traffic on port 80.

{% tip %}
A zone ingress is not an API gateway. It is only used for cross-zone communication within a mesh. API gateways are supported in Kuma [gateway mode](/docs/{{ page.version }}/production/gateway) and can be deployed in addition to zone ingresses.

For Kubernetes the `kuma.io/service` is automatically generated as explained in the [data-plane on Kubernetes documentation](/docs/{{ page.version }}/production/dp/kubernetes-dp).

For load-balancing the zone ingress endpoints are weighted with the number of instances running behind them (.i.e: a zone with 2 instances will receive twice more traffic than a zone with 1 instance), you can also favor local traffic with [localityAware load-balancing](/docs/{{ page.version }}/features/multi-zone/locality-aware).

The `<kuma.io/service>.mesh:80>` is a convention. [Virtual Outbound](/docs/{{ page.version }}/production/networking/outbound-communication)s will enable you to expose hostname/port differently. 
{% endtip %}
...
1.  Find the external IP and port of the `global-remote-sync` service in the `kuma-system` namespace:
    ...
    By default, it's exposed on [port 5685](/docs/{{ page.version }}/production/networking/default-ports). In this example the value is `35.226.196.103:5685`. You pass this as the value of `<global-kds-address>` when you set up the zone control planes.
...
2. Generate the zone ingress token:
   ...
   You can also generate the token [with the REST API](/docs/{{ page.version }}/features/multi-zone/zone-ingress-auth).
...
5. Generate the zone token with `egress` in scope:
   ...
   You can also generate the token [with the REST API](/docs/{{ page.version }}/features/multi-zone/zone-egress-auth).
...
mTLS can be configured in your mesh configuration as indicated in the [mTLS section](/docs/{{ page.version }}/features/zero-trust/mutual-tls).
...
You can use [locality-aware load balancing](../features/multi-zone/locality-aware) to keep requests in the same zone.

To send a request to any zone, you can [use the generated `kuma.io/service`](../production/dp/kubernetes-dp/#tag-generation) and [Kuma DNS](../networking/dns.md#dns):
...
To consume the service in a Universal deployment without transparent proxy add the following outbound to your [dataplane configuration](/docs/{{ page.version }}/production/dp/universal-dp): 
...
Alternatively, if you configure [transparent proxy](/docs/{{ page.version }}/production/networking/transparent-proxying) you can just call `echo-server_echo-example_svc_1010.mesh` without defining an `outbound` section.
...
If you want to have the same service running on both Universal and Kubernetes make sure to align the Universal's data plane inbound to have the same `kuma.io/service` as the one in Kubernetes or leverage [TrafficRoute](/docs/{{ page.version }}/features/traffic/route).
{% endtip %}
...
{% tip %}
With the right resiliency setup ([Retries](/docs/{{ page.version }}/features/health-check/retry), [Probes](/docs/{{ page.version }}/features/health-check/health-check-policy), [Locality Aware LoadBalancing](/docs/{{ page.version }}/features/multi-zone/locality-aware), [Circuit Breakers](/docs/{{ page.version }}/features/performance/circuit-breaker)) the failing zone can be quickly severed and traffic re-routed to another zone.
{% endtip %}
