...
{% if_version gte:2.9.x %}
#### Edge case: same-namespace consumer defaults override producer policies

On Kubernetes, when a producer policy and a consumer policy are both defined in the same namespace,
the consumer policy still has higher priority.
This happens because policy role is evaluated before the specificity of `spec.to[].targetRef`.

For example, this producer policy targets a specific service:
...
A broader consumer default in the same namespace can still override it:
...
If you want producer defaults for services in a namespace to remain in effect,
avoid defining a competing consumer default that targets `Mesh` in that same namespace.
{% endif_version %}
