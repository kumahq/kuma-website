...
Moreover, Zone Egress is a simple proxy that uses a long-living L4 connection to Zone Ingress. Consequently, when a new `MeshLoadbalancingStrategy` with locality awareness is configured, connections won’t be refreshed, and locality awareness will apply only to new connections.
