...
To target other non-mesh traffic, i.e. [passthrough traffic](/docs/{{ page.release }}/networking/non-mesh-traffic#outgoing), use `Mesh` as the `targetRef` kind. In this case, `%KUMA_DESTINATION_SERVICE%` is set to `external`.
