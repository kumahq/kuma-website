...
4. **Prepare a Dataplane resource template**
   ...
   Create a reusable {{ Dataplane }} resource template for services with workload labels:
   ...
   This template includes the `kuma.io/workload` label, which enables {{ Kuma }} to automatically create {{ Workload }} resources for health tracking and workload-based identity assignment.
...
5. **Verify Workload resource**

   {{ Kuma }} automatically creates a {{ Workload }} resource when a {{ Dataplane }} has the `kuma.io/workload` label. This resource represents a logical grouping of data plane proxies that share the same workload identifier and tracks their health status.

   Check the created Workload:

   ```sh
   kumactl get workloads
   ```

   You should see the `kv` workload listed. To view detailed information including the health status:

   ```sh
   kumactl get workload kv -o yaml
   ```

   The output shows the workload status with three key metrics:
   - `connected`: Number of connected data plane proxies
   - `healthy`: Number of healthy proxies
   - `total`: Total number of proxies with this workload identifier

   For the `kv` service, you should see `1` for all three values, indicating a single healthy proxy is running.

{% tip %}
**Note**: Workload resources are fully managed by the control plane.
They are automatically created when Dataplanes with `kuma.io/workload` labels connect and deleted when no matching Dataplanes exist.
For more information, see the {{ Workload }} documentation.
{% endtip %}
...
4. **Verify the application**
   ...
   Verify the `demo-app` {{ Workload }} was created:

   ```sh
   kumactl get workload demo-app -o yaml
   ```
...
- Learn more about {{ MeshIdentity }} in the [MeshIdentity guide]({{ docs }}/guides/meshidentity-guide).
- Explore {{ Workload }} resources in the [Workload documentation]({{ docs }}/resources/workload/).
