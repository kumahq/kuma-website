...
{{site.mesh_product_name}} exposes API server on [ports]{% if_version lte:2.1.x %}(/docs/{{ page.release }}/networking/networking){% endif_version %}{% if_version gte:2.2.x %}(/docs/{{ page.release }}/production/use-mesh#control-plane-ports){% endif_version %} `5681` and `5682` (protected by TLS).
