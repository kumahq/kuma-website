...
(their [effective](/docs/{{ page.release }}/policies/targetref/#merging-configuration) action is **not** `deny`).
...
- [top](/docs/{{ page.release }}/policies/targetref/#target-resources) level `MeshSubset` and `MeshServiceSubset` with `k8s.kuma.io/namespace`, `k8s.kuma.io/service`, `k8s.kuma.io/port` tags
- [from](/docs/{{ page.release }}/policies/targetref/#target-resources) level `MeshSubset` and `MeshServiceSubset` with all tags
