...
[MeshIdentity](/docs/{{ page.release }}/policies/meshidentity) resources automatically create corresponding `MeshTrust` resources when `meshTrustCreation: Enabled` is configured. These autogenerated resources can be identified by the `origin` field in the specification, which contains a `kri` reference to the source MeshIdentity.
