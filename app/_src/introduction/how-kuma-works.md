...
{% tip %}
{{site.mesh_product_name}} ships with an executable `kuma-dp` that executes the bundled `envoy` executable to create the data plane proxy. For details, see the [Overview]{%if_version lte:2.1.x %}(/docs/{{ page.release }}/introduction/what-is-kuma){%endif_version%}{%if_version gte:2.2.x %}(/docs/{{ page.release }}/introduction/overview-of-kuma){%endif_version%}.
{% endtip %}
