...
Once downloaded, you will find the contents of Kuma in the `kuma-{{ page.latestRelease }}` folder. In this folder, you will find - among other files - the `bin` directory that stores the executables for Kuma, including the CLI client [`kumactl`](/docs/{{ page.version }}/documentation/cli/#kumactl).
