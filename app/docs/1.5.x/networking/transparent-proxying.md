...
On **Universal** `kuma-dp` leverages the [data plane proxy specification](/docs/{{ page.release }}/documentation/dps-and-data-model/) associated to it for receiving incoming requests on a pre-defined port.
...
Kuma comes with [`kumactl` executable](/docs/{{ page.release }}/documentation/cli/#kumactl) which can help us to prepare the host. Due to the wide variety of Linux setup options, these steps may vary and may need to be adjusted for the specifics of the particular deployment.
