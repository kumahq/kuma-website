...
On **Universal** `kuma-dp` leverages the [data plane proxy specification](/docs/{{ page.release }}/reference/dpp-specification) associated to it for receiving incoming requests on a pre-defined port.

There are several advantages for using transparent proxying in universal mode:

 * Simpler [Dataplane resource](/docs/{{ page.release }}/reference/dpp-specification), as the `outbound` section becomes obsolete and can be skipped.
...
{% tabs intercepted-tradffic useUrlFragment=false %}
{% tab intercepted-tradffic Kubernetes %}
...
{% endtab %}
{% tab intercepted-tradffic Universal %}
