...
You can learn more about sidecar injection in the section on [Dataplanes](/docs/{{ page.version }}/explore/dpp-on-kubernetes).
...
See also [a complete list of the annotations you can specify in Kubernetes mode](/docs/{{ page.version }}/reference/kubernetes-annotations/).
...
Once the `kuma-cp` process is started, it waits for [data-planes](/docs/{{ page.version }}/explore/dpp) to connect, while at the same time accepting user-defined configuration to start creating Service Meshes and configuring the behavior of those meshes via Kuma [Policies](/docs/{{ page.version }}/policies/introduction).
