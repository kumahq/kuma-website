...
{% tip %}
**Note**: By default this will run Kuma with a `memory` [store](/docs/{{ page.release }}/documentation/configuration#store), but you can use a persistent storage like PostgreSQL by updating the `conf/kuma-cp.conf` file.
{% endtip %}
