...
* `kuma.io/zone`: Identifies the zone name in a [multi-zone deployment](/docs/{{ page.release }}/documentation/deployments/). This tag is automatically created and cannot be overwritten.
...
To implement cross-zone communication when Kuma is deployed in a [multi-zone](/docs/{{ page.release }}/documentation/deployments/#multi-zone-mode) mode, there is a new proxy type `ZoneIngress`. These proxies are not attached to any particular workload. Instead, they are bound to that particular zone.
...
The recommended way to deploy a `ZoneIngress` proxy in Kubernetes is to use `kumactl`, or the Helm charts as specified in [multi-zone](/docs/{{ page.release }}/documentation/deployments/#zone-control-plane). It works as a separate deployment of a single-container pod.
