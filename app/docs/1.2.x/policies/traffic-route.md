...
This policy allows us to configure routing rules for the traffic running in our [Mesh](/docs/{{ page.release }}/policies/mesh). This policy provides support for weighted routing and can be used to implement versioning across our services as well as deployment strategies like blue/green and canary.
...
{% tabs traffic-split useUrlFragment=false %}
{% tab traffic-split Kubernetes %}
```yaml
apiVersion: kuma.io/v1alpha1
kind: TrafficRoute
mesh: default
...
{% tabs traffic-rerouting useUrlFragment=false %}
{% tab traffic-rerouting Kubernetes %}
```yaml
apiVersion: kuma.io/v1alpha1
kind: TrafficRoute
mesh: default
...
{% tab traffic-rerouting Universal %}
```yaml
type: TrafficRoute
name: reroute-traffic
mesh: default
...
{% tabs traffic-split-versions useUrlFragment=false %}
{% tab traffic-split-versions Kubernetes %}
```yaml
apiVersion: kuma.io/v1alpha1
kind: TrafficRoute
mesh: default
...
{% tab traffic-split-versions Universal %}
```yaml
type: TrafficRoute
name: api-split
mesh: default
...
{% tabs traffic-modification useUrlFragment=false %}
{% tab traffic-modification Kubernetes %}
```yaml
apiVersion: kuma.io/v1alpha1
kind: TrafficRoute
mesh: default
...
{% tab traffic-modification Universal %}
```yaml
type: TrafficRoute
name: add-header
mesh: default
...
{% tabs outgoing-request useUrlFragment=false %}
{% tab outgoing-request Kubernetes %}
```yaml
apiVersion: kuma.io/v1alpha1
kind: TrafficRoute
mesh: default
...
{% tab outgoing-request Universal %}
```yaml
type: TrafficRoute
name: http-reroute
mesh: default
...
`TrafficRoute` is an [Outbound Connection Policy](/docs/{{ page.release }}/policies/how-kuma-chooses-the-right-policy-to-apply/#outbound-connection-policy).
