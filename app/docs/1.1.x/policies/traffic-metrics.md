...
The above configuration tells `kuma-prometheus-sd` to talk to `Kuma` Control Plane at [grpcs://{{site.mesh_cp_name}}.internal:5676](grpcs://{{site.mesh_cp_name}}.internal:5676) and save the list of dataplanes to `/var/run/kuma-prometheus-sd/kuma.file_sd.json`.
