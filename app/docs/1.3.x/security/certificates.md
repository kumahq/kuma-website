...
- Security between our services, via the [mTLS policy](/docs/{{ page.release }}/policies/mutual-tls/).
...
{% tabs override-autogenerated useUrlFragment=false %}
{% tab override-autogenerated Kubernetes (kumactl) %}
...
kumactl generate tls-certificate \
  --type=server \
  --cp-hostname={{site.mesh_cp_name}}.{{site.mesh_namespace}} \ # adjust the name if you are installing Kuma to other namespace
  --cp-hostname={{site.mesh_cp_name}}.{{site.mesh_namespace}}.svc # adjust the name if you are installing Kuma to other namespace
...
{% endtab %}
{% tab override-autogenerated Kubernetes (HELM) %}
1) Prepare certificates
...
{% endtab %}
{% tab override-autogenerated Universal %}
1) Prepare certificates
Generate TLS pair with a PKI of your choice and store it in PEM-encoded format in `/tmp/tls.crt`, `/tmp/tls.key`. Store the CA that was used to sign this pair in `/tmp/ca.crt`
...
* `type`. The type can be either `dataplane` (default if not specified) or `ingress`. A [zone-ingress proxy](/docs/{{ page.release }}/documentation/dps-and-data-model/#zone-ingress) in a [multi-zone deployment](/docs/{{ page.release }}/documentation/deployments) requires an ingress token.
...
mTLS is _not_ enabled by default. To enable it, we must apply proper settings in [Mesh](/docs/{{ page.release }}/policies/mesh) policy.
...
{% tip %}
When the [mTLS policy](/docs/{{ page.release }}/policies/mutual-tls/) is enabled, data plane proxy certificates are ephemeral: thet are re-created on every data plane proxy restart and never persisted on disk.
{% endtip %}
