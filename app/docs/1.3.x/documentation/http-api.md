...
{% tip %}
**CI/CD**: The HTTP API can be used for infrastructure automation to either retrieve data, or to make changes when running in Universal mode. The [`kumactl`](/docs/{{ page.release }}/kumactl) CLI is built on top of the HTTP API, which you can also access with any other HTTP client like `curl`.
{% endtip %}
...
{% tip %}
The [`kumactl`](/docs/{{ page.release }}/kumactl) CLI under the hood makes HTTP requests to this API.
{% endtip %}
