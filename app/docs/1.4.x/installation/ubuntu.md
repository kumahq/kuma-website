...
{% tabs ubuntu-run useUrlFragment=false %}
{% tab ubuntu-run Standalone %}
...
{% tabs ubuntu-use useUrlFragment=false %}
{% tab ubuntu-use GUI (Read-Only) %}
...
{% endtab %}
{% tab ubuntu-use HTTP API (Read & Write) %}
...
{% endtab %}
{% tab ubuntu-use kumactl (Read & Write) %}
