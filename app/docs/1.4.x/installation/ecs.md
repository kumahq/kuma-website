...
The services within the Kuma mesh are exposed through their names (as defined in the `kuma.io/service` tag) in the [`.mesh` DNS zone](/docs/{{ page.release }}/networking/dns/#data-plane-proxy-dns). In the default workload example presented in these instructions, our services will be available on `httpbin.mesh`.
...
In order to run the `kuma-dp` container, we have to issue an access token. The latter can be generated using the Admin API of the Kuma CP. Learn more about [DP and CP security](/docs/{{ page.release }}/security/certificates/#data-plane-proxy-to-control-plane-communication).
...
The generated token is valid for all data plane proxies in the `default` mesh. Kuma also allows to generate data plane proxy token in a more restrictive way and [bound to its name or tags](/docs/{{ page.release }}/security/dp-auth/#data-plane-proxy-token-boundary).

{% tip %}
Kuma allows much more advanced and secure ways to expose the `/tokens` endpoint. The full procedure is described in the following security documentation: [data plane proxy authentication](/docs/{{ page.release }}/security/certificates/#data-plane-proxy-to-control-plane-communication), [user to control plane communication](/docs/{{ page.release }}/security/certificates/#user-to-control-plane-communication).
{% endtip %}
