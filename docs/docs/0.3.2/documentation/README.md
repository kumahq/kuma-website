...
If you haven't read the first [Welcome to Kuma](/docs/0.3.2) section, we strongly suggest to start from here.

## Overview

As we have [already learned](/docs/0.3.2), Kuma is a universal control plane that can run across both modern environments like Kubernetes and more traditional VM-based ones.
...
* **Control-Plane**: Kuma is first and foremost a control-plane that will accept user input (you are the user) in order to create and configure [Policies](/docs/0.3.2/policies) like [Service Meshes](/docs/0.3.2/policies/#mesh), and in order to add services and configure their behavior within the Meshes you have created.
...
::: tip
**Multi-Mesh**: Kuma ships with multi-tenancy support since day one. This means you can create and configure multiple isolated Service Meshes from **one** control-plane. By doing so we lower the complexity and the operational cost of supporting multiple meshes. [Explore Kuma's Policies](/docs/0.3.2/policies).
:::
...
::: tip
This information has been documented for clarity, but when following the [installation instructions](/install/0.3.2) these settings will be automatically configured.
:::
...
[Install Kuma](/install/0.3.2) and follow the instructions to get up and running in a few steps.
...
* There must exist at least one [`Mesh`](/docs/0.3.2/policies/#mesh) in Kuma. By default the system auto-generates a `default` Mesh when the control-plane is run for the first time.
...
Tags are important because can be used later on by any [Policy](/docs/0.3.2/policies) that Kuma supports now and in the future. For example, it will be possible to route requests from one region to another assuming there is a `region` tag associated to the data-planes.
...
* Retrieve the state of Kuma and the configured [policies](/docs/0.3.2/policies) in every environment.
...
mTLS is _not_ enabled by default. To enable it, apply proper settings in [Mesh](/docs/0.3.2/policies/#mesh) policy.
...
#### TrafficPermission
When mTLS is enabled, every connection between dataplanes is denied by default, so you have to explicitly allow it using [TrafficPermission](/docs/0.3.2/policies/#traffic-permissions).
