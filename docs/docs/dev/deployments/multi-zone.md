...
:::tip
A zone ingress is not an API gateway. Instead, it is specific to internal cross-zone communication within the mesh. API gateways are supported in Kuma [gateway mode](../explore/dpp.md) which can be deployed in addition to zone ingresses.
:::
