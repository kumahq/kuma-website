...
::: warning
Zone token can currently be used only to identify zone egresses, which means
...
When it'll happen zone ingress tokens and dataplane tokens will become
deprecated. 
:::
...
If the signing key is compromised, we must rotate it and all the tokens that was
signed by it.
...
2. Regenerate tokens
   Create new Zone tokens. These tokens are automatically created with
...
When running in multi-zone, mode we can generate Zone token only on global
control plane.
