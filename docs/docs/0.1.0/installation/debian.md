...
You can download Kuma from [here](https://kong.bintray.com/kuma/:kuma-0.1.0-debian.tar.gz) or by running:

```sh
$ wget https://kong.bintray.com/kuma/kuma-0.1.0-debian.tar.gz
```

You can extract the archive and check the contents of the `bin` folder by running:

```sh
$ tar xvzf kuma-0.1.0-debian.tar.gz
$ cd bin/ && ls
