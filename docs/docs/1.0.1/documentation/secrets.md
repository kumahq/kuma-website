...
Secrets belong to a specific [`Mesh`](/docs/1.0.1/policies/mesh) resource, and cannot be shared across different `Meshes`.
...
Kubernetes Secrets always belongs to a specific [`Mesh`](/docs/1.0.1/policies/mesh) resource and they are internally they are identified with the `name + namespace` format, therefore **it is not possible** to have a `Secret` with the same name in multiple meshes (since multiple `Meshes` always belong to one Kuma CP that always runs in one Namespace).
...
Here is example of how you can use a Kuma `Secret` with a `provided` [Mutual TLS](/docs/1.0.1/policies/mutual-tls) backend.
