...
* [builtin](#usage-of-builtin-ca): it automatically auto-generates a CA root certificate and key, that are also being automatically stored as a [Secret](/docs/1.0.1/documentation/secrets).
* [provided](#usage-of-provided-ca): the CA root certificate and key are being provided by the user in the form of a [Secret](/docs/1.0.1/documentation/secrets).
...
When using a `builtin` backend Kuma automatically generates a root CA certificate and key that are being stored as a Kuma [Secret resource](/docs/1.0.1/documentation/secrets) with the following name:

* `{mesh name}.ca-builtin-cert-{backend name}` for the certificate
* `{mesh name}.ca-builtin-key-{backend name}` for the key

On Kubernetes, Kuma secrets are being stored in the `kuma-system` namespace, while on Universal they are being stored in the underlying [backend](/docs/1.0.1/documentation/backends) configured in `kuma-cp`.
...
Unlike the `builtin` backend, with `provided` we need to first upload the certificate and key that Kuma will use as [Secret resources](/docs/1.0.1/documentation/secrets), and then reference the Secrets to the mTLS configuration.
...
First we need to upload our CA root certificate and key as [Kuma Secrets](/docs/1.0.1/documentation/secrets) so that we can later reference them.
...
The command will generate a certificate at `crt.pem` and the key at `key.pem`. We can generate the Kuma Secret resources by following the [Secret reference](/docs/1.0.1/documentation/secrets).
...
We can use the Kuma HTTP API by retrieving the [Dataplane Insight](/docs/1.0.1/documentation/http-api/#dataplane-overviews) resource and inspecting the `dataplaneInsight` object.
