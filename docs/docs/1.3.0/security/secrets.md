...
Secrets belong to a specific [`Mesh`](/docs/1.3.0/policies/mesh) resource, and cannot be shared across different `Meshes`.
...
This API requires authentication. Consult [Accessing Admin Server from a different machine](/docs/1.3.0/security/certificates/#user-to-control-plane-communication) how to configure remote access.
...
Here is example of how you can use a Kuma `Secret` with a `provided` [Mutual TLS](/docs/1.3.0/policies/mutual-tls) backend.
