...
You can download Kuma from [here](https://kong.bintray.com/kuma/kuma-DRAFT-centos-amd64.tar.gz) or by running:

```sh
$ wget https://kong.bintray.com/kuma/kuma-DRAFT-centos-amd64.tar.gz
```

You can extract the archive and check the contents of the `bin` folder by running:

```sh
$ tar xvzf kuma-DRAFT-centos-amd64.tar.gz
$ cd bin/ && ls
