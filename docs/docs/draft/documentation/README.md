...
in the system. On Kubernetes, certificates are stored as a [Kubernetes Secret](https://kubernetes.io/docs/concepts/configuration/secret/).
On Universal, we leverage the same storage (Postgres) that is used for storing policies.

Certificates generated by Kuma are X.509 certificates that are [SPIFFE](https://github.com/spiffe/spiffe/blob/master/standards/X509-SVID.md) compliant. The SAN of certificate is set to `spiffe://<mesh name>/<service name>`
...
4) Configure `kumactl` with client certificate. `kumactl config control-planes add --name local-kuma --address http://kumacp.internal:5681 --dataplane-token-client-cert cert.pem --dataplane-token-client-key key.pem`

### mTLS

Once the connection between the dataplane and SDS server is established, the dataplane can now fetch its certificate
and certificates of dependant services' dataplanes. When establishing a connection between services each side validates
each other dataplane certificate confirming the identity. Every connection is denied by default, so you have to explicitly allow it using [TrafficPermission](/docs/DRAFT/policies/#traffic-permissions).
