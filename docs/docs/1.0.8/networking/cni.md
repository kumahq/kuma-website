...
to pass through the proxy itself. This is done through [transparen proxying](/docs/1.0.8/networking/transparent-proxying), which is set up automatically on Kubernetes. Installing it requires certain privileges, which are delegated to pre-sidecar initialisation steps.
