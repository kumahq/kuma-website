...
::: tip
**Don't forget!** The [Official Documentation](/docs/0.3.1/) of Kuma is a great place to learn about both basic and more advanced topics.
:::
...
You can download Kuma from [here](https://kong.bintray.com/kuma/kuma-0.3.1-debian-amd64.tar.gz) or by running:

```sh
$ wget https://kong.bintray.com/kuma/kuma-0.3.1-debian-amd64.tar.gz
```

You can extract the archive and check the contents of the `bin` folder by running:

```sh
$ tar xvzf kuma-0.3.1-debian-amd64.tar.gz
$ cd bin/ && ls
...
Kuma automatically creates a [`Mesh`](/docs/0.3.1/policies/#mesh) entity with name `default`. 
...
We now have our control-plane and services running. For each service we can now provision a [`Dataplane Entity`](/docs/0.3.1/documentation/#dataplane-entity) that configures the inbound and outbound networking configuration:
...
You can now review the entities created by Kuma by using the [`kumactl`](/docs/0.3.1/documentation/#kumactl) CLI. For example you can list the Meshes:
