...
* Set `controlPlane.tls.general.secret` to `general-tls-certs` and `controlPlane.tls.general.caBundle` to `<BASE64 content of ca.crt>` via HELM
...
* Set `controlPlane.tls.apiServer.secret` to `api-server-tls` via HELM
:::
::: tab "Universal"
To override autogenerated certificates set the following configuration
...
* Set `controlPlane.tls.apiServer.clientCertsSecret` to `api-server-client-certs` via HELM
:::
::: tab "Universal"
