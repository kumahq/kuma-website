...
- Security between our services, via the [mTLS policy](/docs/1.2.1/policies/mutual-tls/).
...
:::tip
This section is not to be confused with the [mTLS policy](/docs/1.2.1/policies/mutual-tls/) that we can apply to a [Mesh](/docs/1.2.1/policies/mesh/) to secure service-to-service traffic.
:::
...
* `type`. The type can be either `dataplane` (default if not specified) or `ingress`. A [zone-ingress proxy](/docs/1.2.1/documentation/dps-and-data-model/#zone-ingress) in a [multi-zone deployment](/docs/1.2.1/documentation/deployments/) requires an ingress token.
...
:::tip
When the [mTLS policy](/docs/1.2.1/policies/mutual-tls/) is enabled, data plane proxy certificates are ephemeral: thet are re-created on every data plane proxy restart and never persisted on disk.
:::
