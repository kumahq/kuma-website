...
You can download Kuma from [here](https://kong.bintray.com/kuma/kuma-0.2.0-ubuntu-amd64.tar.gz) or by running:

```sh
$ wget https://kong.bintray.com/kuma/kuma-0.2.0-ubuntu-amd64.tar.gz
```

You can extract the archive and check the contents of the `bin` folder by running:

```sh
$ tar xvzf kuma-0.2.0-ubuntu-amd64.tar.gz
$ cd bin/ && ls
...
Kuma automatically creates a [`Mesh`](/docs/0.2.0/policies/#mesh) entity with name `default`. 

By default this will run Kuma with a `memory` [backend](/docs/0.2.0/documentation/#backends), but you can change this to use PostgreSQL by updating the `conf/kuma-cp.conf` file.
...
We now have our control-plane and services running. For each service we can now provision a [`Dataplane Entity`](/docs/0.2.0/documentation/#dataplane-entity) that configures the inbound and outbound networking configuration:
