...
* [macOS](https://kong.bintray.com/kuma/kuma-0.7.0-darwin-amd64.tar.gz) or run `brew install kumactl`
...
Once downloaded, you will find the contents of Kuma in the `kuma-0.7.0` folder. In this folder, you will find - among other files - the `bin` directory that stores the executables for Kuma, including the CLI client [`kumactl`](/docs/0.7.0/documentation/kumactl/).
...
In order to start using Kuma, it's time to check out the [quickstart guide for Kubernetes](/docs/0.7.0/quickstart/kubernetes/) deployments.
