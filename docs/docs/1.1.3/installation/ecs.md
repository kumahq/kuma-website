...
First we need to download the scripts that will setup our environment. The scripts are stored in the main GitHub repository of Kuma in the [examples folders](https://github.com/kumahq/kuma/tree/1.1.2/examples/ecs).

To download the scripts locally:

```shell
$ curl --location --output - https://github.com/kumahq/kuma/archive/1.1.2.tar.gz | tar -z --strip 3 --extract --file=- "./kuma-1.1.2/examples/ecs/*yaml"
```
...
A Kuma [`ingress` data plane proxy](/docs/1.1.2/documentation/dps-and-data-model/#ingress) is needed in each zone to enable cross-zone communication. Like every other data plane proxy type, it also needs a [data plane proxy token](/docs/1.1.2/installation/ecs/#generate-the-dp-token) if the data plane proxy and control plane communication is secured. Learn more about [DP and CP security](/docs/1.1.2/documentation/security/#data-plane-proxy-to-control-plane-communication).
...
The services within the Kuma mesh are exposed through their names (as defined in the `kuma.io/service` tag) in the [`.mesh` DNS zone](/docs/1.1.2/documentation/networking/#kuma-dns). In the default workload example presented in these instructions, our services will be available on `httpbin.mesh`. 
...
In order to run the `kuma-dp` container, we have to issue an access token. The latter can be generated using the Admin API of the Kuma CP. Learn more about [DP and CP security](/docs/1.1.2/documentation/security/#data-plane-proxy-to-control-plane-communication).
...
The generated token is valid for all data plane proxies in the `default` mesh. Kuma also allows to generate data plane proxy token in a more restrictive way and [bound to its name or tags](https://kuma.io/docs/1.1.2/documentation/security/#data-plane-proxy-authentication).

:::tip
Kuma allows much more advanced and secure ways to expose the `/tokens` endpoint. The full procedure is described in the following security documentation: [data plane proxy authentication](https://kuma.io/docs/1.1.2/documentation/security/#data-plane-proxy-to-control-plane-communication), [user to control plane communication](https://kuma.io/docs/1.1.2/documentation/security/#user-to-control-plane-communication).
:::
