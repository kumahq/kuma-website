...
* [builtin](#usage-of-builtin-ca): it automatically auto-generates a CA root certificate and key, that are also being automatically stored as a [Secret](/docs//security/secrets/).
...
Unlike the `builtin` backend, with `provided` you first upload the certificate and key as [Secret resources](/docs//documentation/secrets), and then reference the Secrets in the mTLS configuration.
