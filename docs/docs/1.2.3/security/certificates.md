...
- Security between our services, via the [mTLS policy](/docs//policies/mutual-tls/).
...
* `type`. The type can be either `dataplane` (default if not specified) or `ingress`. A [zone-ingress proxy](/docs//documentation/dps-and-data-model/#zone-ingress) in a [multi-zone deployment](/docs//documentation/deployments/) requires an ingress token.
