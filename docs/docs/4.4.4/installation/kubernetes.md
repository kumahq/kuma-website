...
::: tip
Kuma also provides [Helm charts](/docs/4.4.4/installation/helm/) that we can use instead of this distribution.
:::
...
* [macOS](https://download.konghq.com/mesh-alpine/kuma-4.4.4-darwin-amd64.tar.gz) or run `brew install kumactl`
...
Once downloaded, you will find the contents of Kuma in the `kuma-4.4.4` folder. In this folder, you will find - among other files - the `bin` directory that stores the executables for Kuma, including the CLI client [`kumactl`](/docs/4.4.4/documentation/cli/#kumactl).
...
In order to start using Kuma, it's time to check out the [quickstart guide for Kubernetes](/docs/4.4.4/quickstart/kubernetes/) deployments.
