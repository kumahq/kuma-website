...
It is also used for _administrative_ actions such as
* Managing Kuma Secrets (on Universal)
* Generating Dataplane Tokens
* Generating User Tokens

Administrative actions requires authentication as an admin.

## Roles

Kuma by default recognize an admin by
* `admin` username
* `admin` group membership

You can change those values by overriding `KUMA_RBAC_STATIC_ADMIN_USERS` or `KUMA_RBAC_STATIC_ADMIN_GROUPS`.
...
* The token’s serial number, used for token's signing key rotation
...
```sh
$ kubectl get secret %s -n %s --template={{.data.value}} | base64 -d
```
...
By default, all requests that originates from localhost are authenticated as user of name `admin` that belongs to group `admin`.
After you retrieve and store the admin token, it is recommended to set `KUMA_API_SERVER_AUTHN_LOCALHOST_IS_ADMIN` to `false`.
:::
::::
...
Set `KUMA_API_SERVER_AUTHN_TOKENS_BOOTSTRAP_ADMIN_TOKEN` to `false`.
:::
::: tab "Universal"
...
```
eyJhbGciOiJIUzI1NiIsImtpZCI6IjEiLCJ0eXAiOiJKV1QifQ.eyJOYW1lIjoiam9obiIsIkdyb3VwcyI6WyJ0ZWFtLWEiXSwiZXhwIjoxNjM0MDMxMDkwLCJuYmYiOjE2MzM5NDQzOTAsImlhdCI6MTYzMzk0NDY5MCwianRpIjoiYzU5OTBlZDYtM2NkOS00ODczLTg3NDUtYjk2ZTcxODcwOThjIn0.-Zv4RdlGpRHyM83wv6C5LZrxVpa46IKa10xkLS88OqU
```
...
https://jwt.io/#id_token=eyJhbGciOiJIUzI1NiIsImtpZCI6IjEiLCJ0eXAiOiJKV1QifQ.eyJOYW1lIjoiam9obiIsIkdyb3VwcyI6WyJ0ZWFtLWEiXSwiZXhwIjoxNjM0MDMxMTI2LCJuYmYiOjE2MzM5NDQ0MjYsImlhdCI6MTYzMzk0NDcyNiwianRpIjoiOWI0MDM2M2YtOTBmMC00ODBhLWJlNGUtNjVkZjQwNjlhZGI4In0.7G1ELhAf2V9kkPAR_5LQa1N8_3G7wW6lQQUVYWulUzc
...
✻ Payload
{
  ...
  "nbf": 1633944426,
  "iat": 1633944726,
  "jti": "9b40363f-90f0-480a-be4e-65df4069adb8"
}
   Issued At: 1633944726 10/11/2021, 11:32:06 AM
   Not Before: 1633944426 10/11/2021, 11:27:06 AM
   Expiration Time: 1634031126 10/12/2021, 11:32:06 AM

✻ Signature 7G1ELhAf2V9kkPAR_5LQa1N8_3G7wW6lQQUVYWulUzc
```
...
To use admin client certificates, set `KUMA_API_SERVER_AUTHN_TYPE` to `clientCerts`.
