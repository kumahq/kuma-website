...
::: tip
Kuma also provides [Helm charts](/docs/1.4.0/installation/helm/) that we can use instead of this distribution.
:::
...
* [macOS](https://download.konghq.com/mesh-alpine/kuma-1.4.0-darwin-amd64.tar.gz) or run `brew install kumactl`
...
Once downloaded, you will find the contents of Kuma in the `kuma-1.4.0` folder. In this folder, you will find - among other files - the `bin` directory that stores the executables for Kuma, including the CLI client [`kumactl`](/docs/1.4.0/documentation/cli/#kumactl).
...
In order to start using Kuma, it's time to check out the [quickstart guide for Kubernetes](/docs/1.4.0/quickstart/kubernetes/) deployments.
