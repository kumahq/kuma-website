...
::: tip
Kuma also provides [Helm charts](/docs/1.0.7/installation/helm/) that we can use instead of this distribution.
:::
...
* [macOS](https://kong.bintray.com/kuma/kuma-1.0.7-darwin-amd64.tar.gz) or run `brew install kumactl`
...
Once downloaded, you will find the contents of Kuma in the `kuma-` folder. In this folder, you will find - among other files - the `bin` directory that stores the executables for Kuma, including the CLI client [`kumactl`](/docs/1.0.7/documentation/cli/#kumactl).
...
In order to start using Kuma, it's time to check out the [quickstart guide for Kubernetes](/docs/1.0.7/quickstart/kubernetes/) deployments.
