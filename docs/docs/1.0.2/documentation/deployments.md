...
Once Kuma is up and running, data plane proxies can now [connect](/docs//documentation/dps-and-data-model) directly to it. 
...
:::tip
An `ingress` data plane is specific to internal communication within a mesh and it is not to be considered an API gateway. API gateways are supported via Kuma's [gateway mode](/docs//documentation/dps-and-data-model/#gateway) which can be deployed **in addition** to `ingress` data planes.
:::
