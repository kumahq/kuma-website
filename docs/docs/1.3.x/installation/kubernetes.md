...
::: tip
Kuma also provides [Helm charts](/docs/1.3.1/installation/helm/) that we can use instead of this distribution.
:::
...
Once downloaded, you will find the contents of Kuma in the `kuma-1.3.1` folder. In this folder, you will find - among other files - the `bin` directory that stores the executables for Kuma, including the CLI client [`kumactl`](/docs/1.3.1/documentation/cli/#kumactl).
...
In order to start using Kuma, it's time to check out the [quickstart guide for Kubernetes](/docs/1.3.1/quickstart/kubernetes/) deployments.
